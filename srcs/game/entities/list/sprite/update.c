...
/*   Updated: 2020/07/20 19:33:32 by lbenard          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	sprite_entity_update(t_sprite_entity *const self)
{
	...
	self->end_x = sprite_screen_x + sprite_size.x / 2;
	self->start_y = self->frame_ref->size.y / 2 - sprite_size.y / 2
			+ (ssize_t)self->player_ref->super.transform.rotation.x;
	self->end_y = self->frame_ref->size.y / 2 + sprite_size.y / 2
		+ (ssize_t)self->player_ref->super.transform.rotation.x;
