...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   layer_transform_add.c                              :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: lbenard <lbenard@student.42.fr>            +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/07/11 21:07:52 by lbenard           #+#    #+#             */
/*   Updated: 2020/07/11 21:08:54 by lbenard          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
								...
								const t_frame_transform transform)
{
	...
	while (i.y < normalized.end.y)
	{
		...
		{
			fg_pixel = layer->pixels[position(layer, i, layer_coordinates)];
			fg_pixel.c.a *= (float)transform.opacity / 255.0f;
			if (fg_pixel.c.a == 255)
				self->pixels[self->size.x * i.y + i.x] = fg_pixel;
			else
				self->pixels[self->size.x * i.y + i.x] =
